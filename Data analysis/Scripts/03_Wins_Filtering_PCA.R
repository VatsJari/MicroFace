# ============================================================================
# Cell Filtering and PCA Analysis Script
# ============================================================================
# This script performs quality control filtering, outlier removal, and PCA
# on microglial morphology data. All outputs are stored in the 'Filter' and
# 'PCA' lists for organized access.
# ============================================================================
...
if (!exists("import") || is.null(import$df_all_PCA)) {
  stop("import$df_all_PCA not found. Please run the data import script first.")
}
...
# Winsorize function to handle outliers (1% trimming on both tails)
...
# Apply winsorization to all feature columns
Filter$feat_wins <- Filter$feat_df %>%
  mutate(across(everything(), ~Filter$winsorize(.x, 0.01)))

# Check for and handle NA values (should be none after winsorization, but just in case)
...
# Set cutoff at 99.9% quantile of chi-squared distribution
...
# Combine clean data (original scale, not scaled)
Filter$df_clean_final <- cbind(Filter$meta_clean, Filter$feat_clean)

# Scale features using median and IQR (robust scaling) for PCA
...
# Final dataframe for PCA (metadata + scaled features)
...
# 3. PCA BEFORE FILTERING (first 2 PCs using original data) for comparison
...
# Find most variable features in original data
...
# 6. SUMMARY TABLE PLOT (text-based)
...
Filter$plot_summary <- gridExtra::tableGrob(Filter$summary_table, rows = NULL)
grid::grid.draw(Filter$plot_summary)
...
# Option 2: Save the QC figure for manuscript (commented out)
...
# Get number of PCs to explain 97% variance (original used 0.97)
PCA$n_pcs_97 <- which(cumsum(PCA$var_explained) >= 0.97)[1]
cat("\nNumber of PCs explaining 97% variance:", PCA$n_pcs_97, "\n")
...
# ============================================================================
# PLOTTING - Using base R (fixed margin issues by creating a new device)
# ============================================================================
cat("\n=== GENERATING BASE R PLOTS ===\n")
...
# Add overall title
mtext("PCA Visualizations (Base R)", outer = TRUE, line = 0.5, cex = 1.5, font = 2)

# Reset plotting parameters
par(mfrow = c(1, 1))

# ============================================================================
# ALTERNATIVE: Save base R plots to file (avoids on-screen display issues)
# ============================================================================
cat("\n=== SAVING BASE R PLOTS TO FILE ===\n")

#pdf("PCA_BaseR_Plots.pdf", width = 12, height = 10)
par(mfrow = c(2, 2), mar = c(4, 4, 3, 1), oma = c(1, 1, 2, 1))
...
dev.off()
cat("✓ Saved base R plots to PCA_BaseR_Plots.pdf\n")

# ============================================================================
# GGPLOT VERSION (Better for publication)
# ============================================================================
...
if (requireNamespace("ggplot2", quietly = TRUE) &&
  ...
  # Create a new list for ggplot objects
  PCA$gg <- list()
  ...
  if (length(PCA$gg) == 4) {
    ...
    if (length(PCA$gg) > 0) {
      PCA$gg_combined <- patchwork::wrap_plots(PCA$gg, ncol = 2) +
        plot_annotation(
          title = "PCA Visualizations (ggplot2)",
...
# Save individual ggplot panels as separate files
cat("\n=== SAVING INDIVIDUAL GGPLOTS ===\n")
...
cat("\nTo access results:\n")
cat("  - Filtered data (original scale): Filter$df_clean_final\n")
cat("  - Scaled features: Filter$feat_scaled\n")
cat("  - PCA object: PCA$pca\n")
cat("  - PC scores (first 10): PCA$pca_scores\n")
cat("  - PCA dataframe with metadata: PCA$pca_df\n")
cat("  - ggplot2 PCA plots: PCA$gg (list)\n")
