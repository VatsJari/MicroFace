...
# ============================================================================
# This script performs stratified/random sampling, k-means clustering,
# UMAP dimensionality reduction, and visualization of microglial morphology.
# All outputs are stored in the 'ClusterAnalysis' list.
# ============================================================================
...
ClusterAnalysis$pca_scores_sampled <- PCA$pca$x[ClusterAnalysis$sampled_idx, 1:PCA$n_pcs_97]
...
# ============================================================================
# 4. DETERMINE OPTIMAL K (ELBOW AND SILHOUETTE) - COMMENTED OUT BY USER
# ============================================================================
# The following code is commented out in the original; kept for reference.
# Uncomment if you want to automatically determine optimal k.
...
# ============================================================================
# 5. RUN FINAL CLUSTERING (MANUAL K SET TO 13)
# ============================================================================

# Set optimal k manually (as per original code)
ClusterAnalysis$optimal_k <- 13  # or use ClusterAnalysis$optimal_k_sil if uncommented
...
ClusterAnalysis$meta_sampled$UMAP1 <- ClusterAnalysis$umap_sampled[, 1]
ClusterAnalysis$meta_sampled$UMAP2 <- ClusterAnalysis$umap_sampled[, 2]
...
ggplot(ClusterAnalysis$meta_sampled, aes(UMAP1, UMAP2, color = Cluster))+
  geom_point(size=0.01)+
  scale_color_manual(values = morpho_colours)+
  theme_classic()

names(morpho_colours) <- levels(factor(ClusterAnalysis$meta_sampled$Cluster))

# Compute centroids
centroids_umap <- ClusterAnalysis$meta_sampled %>%
  group_by(Cluster) %>%
  summarise(UMAP1 = mean(UMAP1, na.rm = TRUE),
            UMAP2 = mean(UMAP2, na.rm = TRUE),
            .groups = "drop") %>%
  mutate(Cluster = factor(Cluster, levels = levels(factor(ClusterAnalysis$meta_sampled$Cluster))))

# Assign text color from palette
centroids_umap$text_color <- morpho_colours[as.numeric(centroids_umap$Cluster)]

ClusterAnalysis$umap_cluster <- ggplot(ClusterAnalysis$meta_sampled, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
  geom_point(size = 0.1, alpha = 0.7, shape=".") +
  scale_color_manual(values = morpho_colours) +
  theme_classic() +
  labs(title = "UMAP by Cluster", x = "UMAP1", y = "UMAP2") +
  theme(legend.position = "none")

# Add centroid labels
ClusterAnalysis$umap_cluster <- ClusterAnalysis$umap_cluster +
...
print(ClusterAnalysis$umap_cluster)
...
ClusterAnalysis$summary <- list(
  ...
  # optimal_k_silhouette = ClusterAnalysis$optimal_k_sil,  # uncomment if needed
...
# ============================================================================
# 12. FILTERING CLUSTER 1 (FALSE POSITIVE DETECTION)
# ============================================================================

remove_cluster <-  c(1, 12)

ClusterAnalysis$final_df_sampled <- 
  ClusterAnalysis$final_df_sampled %>% filter(!Cluster %in% remove_cluster)

ClusterAnalysis$final_df_full <- 
  ClusterAnalysis$final_df_full %>% filter(!Cluster %in% remove_cluster)

dim(ClusterAnalysis$final_df_sampled)
dim(ClusterAnalysis$final_df_full)

# ============================================================================
# 12. SAVE RESULTS (Optional, commented out)
# ============================================================================
...
# Save dataframes (uncomment to use)
